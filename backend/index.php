...
// TIC: resetear contrasena de docente
if ($path === '/tic/reset-password-docente' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_input();
    $password = trim($data['password'] ?? '');
    $dni = trim($data['dni'] ?? '');
    ...
    if ($dni === '' || $passwordNueva === '') {
        respond(['ok' => false, 'error' => 'DNI y nueva contrasena son obligatorios.'], 400);
    }
    if (strlen($passwordNueva) < 6) {
        respond(['ok' => false, 'error' => 'La nueva contrasena debe tener al menos 6 caracteres.'], 400);
    }
    $stmt = $pdo->prepare("SELECT id_docente FROM docentes WHERE dni = ?");
    $stmt->execute([$dni]);
    $docente = $stmt->fetch();
    if (!$docente) {
        respond(['ok' => false, 'error' => 'No existe un docente con ese DNI.'], 404);
    }
    $nuevoHash = password_hash($passwordNueva, PASSWORD_DEFAULT);
    $upd = $pdo->prepare("UPDATE docentes SET password_hash = ? WHERE id_docente = ?");
    $upd->execute([$nuevoHash, $docente['id_docente']]);
    respond(['ok' => true, 'mensaje' => 'Contrasena del docente actualizada correctamente.']);
}

// TIC: resetear contrasena de alumno
if ($path === '/tic/reset-password-alumno' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_input();
    $password = trim($data['password'] ?? '');
    $dni = trim($data['dni'] ?? '');
    $passwordNueva = trim($data['password_nueva'] ?? '');
    ...
    $stmt = $pdo->prepare("SELECT id_usuario FROM estudiantes WHERE dni = ?");
    $stmt->execute([$dni]);
    $estudiante = $stmt->fetch();
    if (!$estudiante || !$estudiante['id_usuario']) {
        respond(['ok' => false, 'error' => 'No existe un alumno con cuenta de usuario para ese DNI.'], 404);
    }
    $nuevoHash = password_hash($passwordNueva, PASSWORD_DEFAULT);
    $upd = $pdo->prepare("UPDATE usuarios SET password_hash = ? WHERE id_usuario = ?");
    $upd->execute([$nuevoHash, $estudiante['id_usuario']]);
    respond(['ok' => true, 'mensaje' => 'Contrasena del alumno actualizada correctamente.']);
}
    ...
    if ($path === '/secretaria/cursadas/verificar' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $habilitada = $estado === 'apto' ? 'true' : 'false';
    ...
    // ── Secretaria: listado de alumnos por materia (para actas de cursado) ─────
    if ($path === '/secretaria/actas/alumnos' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $idMateria = (int) ($_GET['id_materia'] ?? 0);
        $anio      = (int) ($_GET['anio'] ?? 0);
        if ($idMateria === 0 || $anio === 0) {
            respond(['ok' => false, 'error' => 'Faltan parametros (id_materia, anio).'], 400);
        }

        $stmtMateria = $pdo->prepare(
            "SELECT m.nombre_materia, m.codigo_materia, m.formato, m.cuatrimestre,
                    c.nombre_carrera
             FROM materias m
        ...
        if (!$materia) {
            respond(['ok' => false, 'error' => 'Materia no encontrada.'], 404);
        }

        $stmtAlumnos = $pdo->prepare(
            "SELECT e.apellido, e.nombre, e.dni,
                    im.estado_secretaria,
                    cu.resultado AS resultado_cursada,
                    to_char(cu.fecha_resultado, 'DD/MM/YYYY') AS fecha_resultado
             FROM inscripciones_materias im
             JOIN estudiantes e ON e.id_estudiante = im.id_estudiante
             LEFT JOIN cursadas cu ON cu.id_estudiante = im.id_estudiante
               AND cu.id_materia = im.id_materia
        ...
        respond(['ok' => true, 'materia' => $materia, 'anio' => $anio, 'alumnos' => $alumnos]);
    }
...
// Docente: cambiar su propia contrasena
if ($path === '/docente/cambiar-password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_input();
    $dni = trim($data['dni'] ?? '');
    $passwordActual = trim($data['password_actual'] ?? '');
    $passwordNueva = trim($data['password_nueva'] ?? '');
    if ($dni === '' || $passwordActual === '' || $passwordNueva === '') {
        respond(['ok' => false, 'error' => 'DNI, contrasena actual y nueva contrasena son obligatorios.'], 400);
    }
    if (strlen($passwordNueva) < 6) {
        respond(['ok' => false, 'error' => 'La nueva contrasena debe tener al menos 6 caracteres.'], 400);
    }
    ...
    $stmt->execute([$dni]);
    $docente = $stmt->fetch();
    if (!$docente || !password_verify($passwordActual, $docente['password_hash'])) {
        respond(['ok' => false, 'error' => 'DNI o contrasena actual incorrectos.'], 401);
    }
    $nuevoHash = password_hash($passwordNueva, PASSWORD_DEFAULT);
    $upd = $pdo->prepare("UPDATE docentes SET password_hash = ? WHERE id_docente = ?");
    $upd->execute([$nuevoHash, $docente['id_docente']]);
    respond(['ok' => true, 'mensaje' => 'Contrasena actualizada correctamente.']);
}
...
if ($path === '/tic/nuevas-materias' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // ── Directivos: login ───────────────────────────────────────────────────
    if ($path === '/directivos/login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $data     = json_input();
        $password = trim($data['password'] ?? '');
        if ($password === getenv('DIRECTIVOS_PASSWORD')) {
            respond(['ok' => true]);
        }
        respond(['ok' => false, 'error' => 'Clave incorrecta.'], 401);
    }

    // ── Directivos: alumnos activos por carrera ─────────────────────────────
    if ($path === '/directivos/alumnos-por-carrera' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $stmt = $pdo->query(
            "SELECT c.nombre_carrera, COUNT(e.id_estudiante) AS total_activos
             FROM carreras c
             LEFT JOIN estudiantes e ON e.id_carrera = c.id_carrera
               AND e.estado_general NOT IN ('rechazado', 'inactivo')
             GROUP BY c.nombre_carrera
             ORDER BY c.nombre_carrera"
        );
        respond(['ok' => true, 'reporte' => $stmt->fetchAll()]);
    }

    // ── Directivos: alumnos por anio ────────────────────────────────────────
    if ($path === '/directivos/alumnos-por-anio' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $stmtGen = $pdo->query(
            "SELECT anio_actual, COUNT(*) AS total
             FROM estudiantes
             WHERE estado_general NOT IN ('rechazado', 'inactivo')
             GROUP BY anio_actual
             ORDER BY anio_actual"
        );
        $stmtCar = $pdo->query(
            "SELECT c.nombre_carrera, e.anio_actual, COUNT(*) AS total
             FROM estudiantes e
             JOIN carreras c ON c.id_carrera = e.id_carrera
             WHERE e.estado_general NOT IN ('rechazado', 'inactivo')
             GROUP BY c.nombre_carrera, e.anio_actual
             ORDER BY c.nombre_carrera, e.anio_actual"
        );
        respond(['ok' => true, 'general' => $stmtGen->fetchAll(), 'por_carrera' => $stmtCar->fetchAll()]);
    }

    // ── Directivos: docentes por carrera ────────────────────────────────────
    if ($path === '/directivos/docentes-por-carrera' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $stmt = $pdo->query(
            "SELECT c.nombre_carrera, COUNT(DISTINCT dm.id_docente) AS total_docentes
             FROM carreras c
             JOIN materias m ON m.id_carrera = c.id_carrera
             JOIN docente_materias dm ON dm.id_materia = m.id_materia
             JOIN docentes d ON d.id_docente = dm.id_docente AND d.activo = TRUE
             GROUP BY c.nombre_carrera
             ORDER BY c.nombre_carrera"
        );
        respond(['ok' => true, 'reporte' => $stmt->fetchAll()]);
    ...
    // ── Directivos: buscar alumnos ──────────────────────────────────────────
    if ($path === '/directivos/alumnos-buscar' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $q         = trim($_GET['q'] ?? '');
        $idCarrera = (int) ($_GET['id_carrera'] ?? 0);
        $anio      = (int) ($_GET['anio'] ?? 0);

        $sql = "SELECT e.apellido, e.nombre, e.dni, c.nombre_carrera, e.anio_actual, e.estado_general
                FROM estudiantes e
                JOIN carreras c ON c.id_carrera = e.id_carrera
                WHERE 1=1";
        $params = [];
    ...
    // ── Directivos: buscar docentes ─────────────────────────────────────────
    if ($path === '/directivos/docentes-buscar' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        $q         = trim($_GET['q'] ?? '');
        $idCarrera = (int) ($_GET['id_carrera'] ?? 0);

        $sql = "SELECT d.id_docente, d.apellido, d.nombre, d.email,
                       STRING_AGG(DISTINCT c.nombre_carrera, ', ' ORDER BY c.nombre_carrera) AS carreras_str
                FROM docentes d
                JOIN docente_materias dm ON dm.id_docente = d.id_docente
                JOIN materias m ON m.id_materia = dm.id_materia
                JOIN carreras c ON c.id_carrera = m.id_carrera
                WHERE d.activo = TRUE";
        $params = [];
        if ($q !== '') {
            $sql .= " AND (d.apellido ILIKE ? OR d.nombre ILIKE ?)";
            $like = '%' . $q . '%';
        ...
        $sql .= " GROUP BY d.id_docente, d.apellido, d.nombre, d.email
                  ORDER BY d.apellido, d.nombre LIMIT 100";

        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        $docentes = $stmt->fetchAll();
        foreach ($docentes as &$doc) {
            $doc['carreras'] = $doc['carreras_str'] ? explode(', ', $doc['carreras_str']) : [];
            unset($doc['carreras_str']);
        }
        unset($doc);
